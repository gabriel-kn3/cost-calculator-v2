...
export default function AppDock() {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      const hit = actions.find((a) => a.hotkey && a.hotkey.toLowerCase() === key);
      if (hit && e.ctrlKey) {
        e.preventDefault();
        hit.onClick?.();
      }
  ...
  const actions = useMemo(() => app.state.dock.actions || [], [app.state.dock.actions]);

  return (
    <>
      <CommandLegendModal open={legendOpen} actions={actions} onClose={() => setLegendOpen(false)} />
      ...
      >
        <Box direction="row" align="center" gap="small" wrap>
          ...
          <Text size="small" color="text-muted">Ctrl+/</Text>
        </Box>
