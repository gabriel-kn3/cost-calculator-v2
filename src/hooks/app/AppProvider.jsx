...
function reducer(state, action) {
  switch (action.type) {
    ...
    case 'dock.setActions':
      return { ...state, dock: { ...state.dock, actions: action.actions || [] } };

    case 'io.open':
      return { ...state, io: { open: true, mode: action.mode, kind: action.kind } };
    case 'io.close':
      return { ...state, io: { ...state.io, open: false } };

    case 'modal.open':
      return { ...state, modal: { open: true, type: action.modalType, data: action.data ?? null } };
    case 'modal.close':
...
export function AppProvider({ children }) {
  ...
  const api = useMemo(() => {
    return {
      ...
      setDockActions: (actions) => dispatch({ type: 'dock.setActions', actions }),

      openIO: ({ kind, mode, onComplete }) => {
        ioCallbackRef.current = typeof onComplete === 'function' ? onComplete : null;
      ...
      openModal: (modalType, data) => dispatch({ type: 'modal.open', modalType, data }),
      closeModal: () => dispatch({ type: 'modal.close' }),

      toast: (message, status = 'normal') => {
